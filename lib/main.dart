...
class _TimeCopAppState extends State<TimeCopApp> with WidgetsBindingObserver {
  ...
  Future<void> pressHandler(Widget? child, BuildContext context) async {
    ...
    int projectId = getSelectedProject().id;
    double requiredScreenWidth = getSelectedProject().width.toDouble();
    double requiredScreenHeight = getSelectedProject().height.toDouble();
    ...
    try {
      ...
      setWidth(requiredScreenWidth);
      setHeight(requiredScreenHeight);
    ...
    print('targetedItemIds: ${getTargetedItemIds}');
        ...
        if (capturedImage != null) {
          ...
          .then((res) {
            ...
            return Dio().post(uploadUrl, data: {
              'items': [
                {
                  'name': 'scrollable_${getCurrentNo()}_${DateTime.now().millisecondsSinceEpoch}',
                  'base64': 'data:image/png;base64,$base64Value',
                  'itemId': getTargetedItemIds()[getCurrentNo()],
                  'relevantAction': '',
                  'projectId': projectId
                }
              ],
  ...
  Future<int> scrollEachItem(
    ...
    final targetedItemIds = getTargetedItemIds();

    print('isScrollable(child): ${isScrollable(child)}');

    if (
      isScrollable(child) == true &&
      targetedItemIds.length > getCurrentNo()
    ) {
      print('Scrollable widget found');

      while (isScrollable(child) == true && targetedItemIds.length > getCurrentNo()) {
              ...
              'items': [
                {
                  'name': 'scrollable_${getCurrentNo()}_${DateTime.now().millisecondsSinceEpoch}',
                  'base64': 'data:image/png;base64,$base64Value',
                  'itemId': targetedItemIds[getCurrentNo()],
  ...
  Future<void> _dialogBuilder(BuildContext context, Widget? child, Function pressHandler) {
    ...
    print('controller.getDialogOpened(): ${getDialogOpened()}');
    ...
    setContext(context);
    setChild(child);
    setSubmitHandlerFunc(pressHandler);
    ...
    Column content = Column(
        ...
        children: [
          ...
          SizedBox(
            ...
            child: Padding(
              ...
              child: DropdownButton<String>(
                ...
                onChanged: (newValue) {
                  final project = projects.firstWhere((element) => element.name == newValue);
          ...
          Visibility(
            visible: getErrorCode() != 0,
            child: Padding(
              padding: const EdgeInsets.only(top: 8.0),
              child: Text(
                getErrorCode() == 0 ? '' :
                getErrorCode() == 1 ? 'Please select a Campaign' :
                getErrorCode() == 2 ? 'Please select a Project' :
                getErrorCode() == 3 ? 'Something is wrong!' :
                // 'The screen size is wrong. It must be ${getWidth()} x ${getHeight()}',
                'The screen size required is ${getWidth()} x ${getHeight()}. Please change your device or consult your designer.',
                style: getErrorCode() == 0 ? const TextStyle(color: Colors.black) : const TextStyle(color: Colors.red),
              ),
            ),
          ),
          Visibility(
  ...
  void setSubmitHandlerFunc(Function newFunc) {
    setState(() => submitHandlerFunc = newFunc);
  }

  void setContext(BuildContext? newContext) {
    setState(() => currentContext = newContext);
  }

  void setChild(Widget? newChild) {
    setState(() => child = newChild);
  }

  void setTargetedItemIds(List<int> newIds) {
    setState(() => targetedItemIds = newIds);
  }

  void setCurrentNo(int newNo) {
    setState(() => currentNo = newNo);
  }

  void setErrorCode(int newErrorCode) {
    setState(() => errorCode = newErrorCode);
  }
  ...
  void setDialogOpened(bool newDialogOpened) {
    ...
    setState(() => dialogOpened = newDialogOpened);
  }
